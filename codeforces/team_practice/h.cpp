...
void parse(string &a, int &i){
	...
	while(i < a.size()){
		string k = "";
		while(isalpha(a[i]))k+=a[i++];
		i++;
		int d = 0;
		while(i < a.size() && isdigit(a[i]))d=(d*10 + (a[i++]-'0'));
		s.pb(k);
		c.pb(d);
		i++;
	}
...
V<V<string>> ans;
void add_level(int &i, int lev = 0){
	if(ans.size() == lev)ans.emplace_back(V<string>());
	ans[lev].emplace_back(s[i]);
	int inps = c[i];
	i++;
