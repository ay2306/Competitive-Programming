...
void build(int node = 1, int start = 1, int end = n){
    if(start == end)return void(tree[node] = PII(arr[start],arr[start]));
    int mid = start + end >> 1;
...
PII query(int l, int r, int node = 1, int start = 1, int end = n){
    if( l > end || r < start )return PII(INT_MAX,INT_MIN);
    if( l <= start && end <= r)return tree[node];
    int mid = start + end >> 1;
    PII p1 = query(l,r,node<<1,start,mid);
    PII p2 = query(l,r,node<<1|1,mid+1,end);
    return op(p1,p2);
}
...
void solve(int test_case){
    ...
    loop(i,1,n+1){
        int lo = 1, hi = i, ans = i;
        while(lo <= hi){
            int mn,mx,mid = lo + hi >> 1;
            tie(mn,mx) = query(mid,i);
            if(mx - mn > k) lo = mid+1;
    ...
    while(res.back().S - res.back().F + 1 != cnt)res.pop_back();
    cout << cnt << " " << res.size() << "\n";
