...
map<vector<int>,int> dp, mndp;
V<int> arr;
int cnt = 0,n,a,b;
int cal(){
    if(SUM(arr) == -n)return 0;
    int mn = INT_MAX;
    if(mndp.count(arr))return mndp[arr];
    // cout << arr << endl;
    loop(i,0,n){
        vector<int> k;
        if(i)k.pb(arr[i-1]);
        k.pb(arr[i]);
        if(i+1<n)k.pb(arr[i+1]);
        if(SUM(k) == -3)continue;
        if(i)arr[i-1] = max(-1,arr[i-1]-b);
        if(i+1<n)arr[i+1] = max(-1,arr[i+1]-b);
        arr[i] = max(-1,arr[i]-a);
        // cout << "  -- " << arr << endl;
        int res = cal();
        if(i)arr[i-1] = k[0];
        if(i+1<n)arr[i+1] = k.back();
        arr[i] = k[!!i];
        if(res < mn)dp[arr] = i;
        mn = min(mn,res);
    }
...
void solve(int test_case){
    ...
    arr.resize(n);
    cin >> arr;
    loop(i,0,n)cnt+=arr[i]>0;
    V<int> ans;
    while(arr[0] >= 0){
        ans.pb(2);
        int i = 1;
        arr[i-1] = max(-1,arr[i-1]-b);
        arr[i+1] = max(-1,arr[i+1]-b);
        arr[i] = max(-1,arr[i]-a);
    }
    while(arr.back() >= 0){
