...
void solve(int test_case){
	...
	p.resize(n+1,-1);
	ll ans = 0;
	for(auto& [x,y]: e)cin >> y.F >> y.S >> x;
	sort(all(e),greater<P<ll,PLL>>());
	V<PLL> q(m);
	for(int i = 0; i < m; ++i){
		cin >> q[i].F;
		q[i].S = i;
	...
	V<ll> cal(m);
	for(auto &i: q){
		while(e.size() && e.back().first <= i.F){
			if(fp(e.back().S.F) != fp(e.back().S.S)){
				int e1 = e.back().S.F;
				int e2 = e.back().S.S;
				ans+=(p[fp(e1)]*1LL*p[fp(e2)]);
				un(e1,e2);
			}
			e.pop_back();
		}
