...
struct CentroidDecomposition{
	...
	void build(int s, int p, char a){
		int n = dfs(s,p);
		int centroid = dfs(s,p,n);
		ans[centroid] = a;
		if(p == -1)p=centroid;
		parent[centroid] = p;
		queue<int> to_del;
		...
		while(to_del.size()){
			int i = to_del.front();
			to_del.pop();
			tree[centroid].erase(i);
			tree[i].erase(centroid);
			build(i,centroid,a+1);
		}
	}
	int dfs(int s, int p){
		sub[s] = 1;
		for(auto &i:tree[s])if(i!=p)sub[s]+=dfs(i,s);
		return sub[s];
	}
	int dfs(int s, int p, int n){
		for(auto &i: tree[s]){
			if(i != p && sub[i] > n/2)return dfs(i,s,n);
		}
		return s;
	}
};
