...
void solve(int test_case){
    ...
    cin >> n >> k;
    set<int> s,a,tar;
    loop(i,0,n)a.emplace(i+1),tar.emplace(i+1);
    vector<int> ans;
    loop(i,0,k){
        int b;
        cin >> b;
        ans.pb(b);
        if(s.size() && *s.begin() < b)return void(cout << "-1");
        else {
            s.emplace(b);
            a.erase(b);
        }
        while(s.size() && *s.begin() == *tar.begin()){
            tar.erase(tar.begin());
            s.erase(s.begin());
        }
    }
    ...
    loop(i,k,n){
        int mn = *a.rbegin();
        if(s.size())mn = *s.begin();
        int b = *prev(a.upper_bound(mn));
        // cout << mn << " " << b << "\n";
        s.emplace(b);
        a.erase(b);
        ans.pb(b);
