...
void solve(int test_case){
	...
	V<V<ll>> a;
	loop(i,0,3){
		a.emplace_back(V<ll>(s[i]+1));
		loop(j,1,s[i]+1)cin>>a.back()[j];
		sort(all(a.back()),greater<ll>());
	}
	ll ans = 0;
	loop(i,1,s[0]+1){
		loop(j,1,s[1]+1){
			loop(k,1,s[2]+1){
				auto &res = dp[i][j][k];
				res = max(res,dp[i-1][j-1][k] + a[0][i]*a[1][j]); 
				res = max(res,dp[i-1][j][k-1] + a[0][i]*a[2][k]); 
				res = max(res,dp[i][j-1][k-1] + a[2][k]*a[1][j]); 
				ans = max(ans,res);
			}
		}
	}
