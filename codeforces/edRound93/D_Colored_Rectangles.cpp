...
void solve(int test_case){
	...
	loop(i,0,3){
        V<ll> k(s[i]+1);
		loop(j,1,s[i]+1)cin >> k[j];
		a.emplace_back(k);
		sort(1+all(a.back()),greater<ll>());
        // cout << a.back() << "\n";
	}
	ll ans = 0;
	loop(i,0,s[0]+1){
		loop(j,0,s[1]+1){
			loop(k,0,s[2]+1){
				auto &res = dp[i][j][k];
				if(i && j)res = max(res,dp[i-1][j-1][k] + a[0][i]*a[1][j]); 
				if(i && k)res = max(res,dp[i-1][j][k-1] + a[0][i]*a[2][k]); 
				if(k && j)res = max(res,dp[i][j-1][k-1] + a[2][k]*a[1][j]); 
				ans = max(ans,res);
			}
		}
	}
