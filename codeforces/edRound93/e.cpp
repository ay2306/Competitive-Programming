...
void solve(int test_case){
	...
	while(n--){
		...
		if(b < 0){
			...
			if(a == 0){
				...
				if(y.count(b)){
					...
					if(e.size()){
						ans-=e.begin()->first;
						if(e.begin()->second)y.emplace(e.begin()->first);
						if(!e.begin()->second)x.emplace(e.begin()->first);
						e.erase(e.begin());
					}
				}else{
					ans-=b;
			...
			if(e.size()){
				ans-=e.begin()->first;
				if(e.begin()->second)y.emplace(e.begin()->first),z--;
				if(!e.begin()->second)x.emplace(e.begin()->first);
				e.erase(e.begin());
			}
			if(a == 0){
		...
		while(e.size() + 1 < z + y.size() + (x.size() > 0)){
			if(x.size() && y.size() > 1){
				if(*x.rbegin() > *y.rbegin()){
					ans+=(*x.rbegin());
					e.emplace(*x.rbegin(),0);
					x.erase(prev(x.end()));
				}else{
					ans+=(*y.rbegin());
					e.emplace(*y.rbegin(),1);
					y.erase(prev(y.end()));
					z++;
				}
