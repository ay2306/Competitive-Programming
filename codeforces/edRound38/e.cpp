...
#define modInv(N) ll inv[N]; void preModInv(){inv[0]=1;inv[1]=1;for(int i = 2; i < N; ++i)inv[i] = mod-mod/i*inv[mod%i]%mod;}
#define fact(N) ll fact[N]; void preFact(){fact[0] = 1,fact[1] = 1; for(int i = 2; i < N; ++i)fact[i]=fact[i-1]*i%mod;}
#define inFact(N) ll ifact[N]; void preiFact(){ifact[0] = 1;ifact[1] = 1; for(int i = 2; i < N; ++i)ifact[i]=ifact[i-1]*inv[i]%mod;}
// pair operation
...
ll ncr(ll n, ll r){
	if(r < 0 || r > n)return 0;
	ll val = fact[n]*ifact[r]%mod*ifact[n-r]%mod;
	debug(n,r,val,fact[n],ifact[r],ifact[n-r]);
	return val;
}
ll a[N];
