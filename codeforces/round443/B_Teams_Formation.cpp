...
void solve(int test_case){
    ...
    deque<PLL> s;
    loop(i,0,n){
        if(s.size() && s.back().first == a[i]){
            ll cnt = s.back().second + 1;
            s.pop_back();
            if(cnt < k)s.emplace_back(a[i],cnt);
            else total-=m*k;
        }else{
    ...
    deque<PLL> left,right,el,er;
    while(s.size()){
        left.emplace_front(s.back());
        right.emplace_back(s.back());
        el.emplace_front(s.back());
        er.emplace_back(s.back());
        s.pop_back();
    }
    if(m == 2){
        left = el,right=er;
        while(left.size() > 0 && right.size() > 0){
            if(left.back().first != right.back().first)break;
            if(left.back().second + right.back().second >= k){
                total -= (m-1) * ((left.back().second+right.back().second) / k * k);
                ll ss = left.back().second + right.back().second; 
                left.pop_back();
                right.pop_back();
                if(ss % k != 0)break;
            }else break;
    ...
    if(left.size() < 2){
        ...
        for(auto i: el){
            if(s.size() && s.back().first == i.first){
                i.S += s.back().S;
                s.pop_back();
            }
            total -= i.S/k*k;
            i.S%=k;
            if(i.S)s.emplace_back(i);
        }
        // cout << el.size() << "\n";
        // for(auto i: s)cout << "(" << i << ") ";
        // cout << endl;
