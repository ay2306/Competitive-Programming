...
void solve(int test_case){
	...
	loop(i,0,n)row.emplace(a,i);
	loop(i,0,m)col.emplace(b,i);
	...
	list<int> ans;
	loop(i,0,a)ans.emplace_back(1);
	loop(i,0,m-a)ans.emplace_back(0);
	cout << "YES\n";
	if(n == m && a == n-1){
		loop(i,0,n){
	...
	// 	i.first--;
	// 	j.first--;
	// 	if(i.first)row.emplace(i);
	// }
	// 	if(j.first)col.emplace(j);
	// loop(i,0,n){
