...
ll ncr(ll n, ll r){
    ...
    return fct[n]*inv[n-r]%mod*inv[r]%mod;
}
...
void solve(int test_case){
    ...
    if(n % 4 == 0){
        loop(i,0,n){
            if(i&1)ans-=ncr((n-2)/2,i/2)*a[i]%mod;
            if(i&1^1)ans+=ncr((n-2)/2,i/2)*a[i]%mod;
            if(ans >= mod)ans-=mod;
            if(ans < 0)ans+=mod;
        }
    }
        ...
        loops(i,0,n,2){
            ans+=ncr((n-1)/2,i/2)*a[i]%mod;
            if(ans >= mod)ans-=mod;
        }
    }
    else if(n % 4 == 2){
        loop(i,0,n){
            ans+=ncr((n-2)/2,i/2)*a[i]%mod;
            if(ans >= mod)ans-=mod;
        }
    }
    else if(n % 4 == 3){
        int ch[3] = {2,1,-1};
        loop(i,0,n){
            if(!i)ans+=ncr((n-3)/2,i/2)*a[i]%mod;
            else ans+=(ch[(i-1)%3]*(ncr((n-3)/2,(i-1)/3 + ((i-1)%3 == 1))*a[i]%mod)+mod)%mod;
            // if(!i)cout << "( " << (n-3)/2 << " , " << i/2 << ")\n";
            // else cout << ch[(i-1)%3] << " ( " << (n-3)/2 << " , " << (i-1)/3 + ((i-1)%3 == 1) << ")\n";
                if(ans >= mod)ans-=mod;
            if(ans < 0)ans+=mod;
        }
    }
