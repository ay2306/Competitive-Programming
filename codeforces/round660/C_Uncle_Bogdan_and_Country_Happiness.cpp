...
pair<PLL,PLL> dfs(int s = 1, int pr = -1){
    ll sum = 0;
    int cnt = 0;
    P<PLL,PLL> ans (mp(0,0),mp(0,1));
    P<PLL,PLL> neg (mp(0,0),mp(0,0));
    for(int i: g[s]){
        if(i == pr)continue;
        cnt++;
        auto res = dfs(i,s);
    ...
    ans.F.S += p[s];
    ll cur = ans.F.F - ans.F.S;
    ll to_change = abs(h[s] - cur); 
    debug("0",s,ans,cur,h[s],to_change);
    if(to_change % 2 == 1){
        debug("1",to_change,ans,s,h[s],p[s]);
        return neg;
    }
    to_change/=2;
    if(h[s] > cur){
        if(to_change > p[s]){
            debug("2",to_change,ans,s,h[s],p[s]);
            return neg;
        }
        ans.F.S-=to_change;
        ans.F.F+=to_change;
        debug("3",to_change,ans,s,h[s],p[s]);
        return ans;
    }else if(h[s] <= cur){
        if(to_change > ans.F.F){
            debug("4",to_change,ans,s,h[s],p[s]);
            return neg;
        }
        ans.F.S+=to_change;
        ans.F.F-=to_change;
        debug("5",to_change,ans,s,h[s],p[s]);
        return ans;
    }
    return neg;
}
...
void solve(int test_case){
    ...
    if(dfs(1).S.S)return void(cout << "YES\n");
    cout << "NO\n";
}
