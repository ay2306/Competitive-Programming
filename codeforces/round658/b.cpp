...
void solve(int test_case){
    ...
    cin >> n;
    loop(i,0,2*n+1){
        loop(j,0,n+1)fill(dp[i][j],dp[i][j]+2,false);
    }
    arr[0] = INT_MAX;
    loop(i,1,2*n+1)cin >> arr[i];
    dp[1][1][0] = true;
    for(int i = 2; i <= 2*n; ++i){
        for(int j = 1; j <= n; ++j){
            if(arr[i-1] < arr[i] && )
            if(j != 1 && dp[i-1][j-1][0])dp[i][j][0]=true;
            if(j != 1 && dp[i-1][j-1][1])dp[i][j][1]=true;
            if(arr[i-1] < arr[i]){
                int rem = i - j;
                if(rem < 0 && rem > n)continue;
                if(dp[i-1][rem][1])dp[i][j][0]=true;
                if(dp[i-1][rem][0])dp[i][j][1]=true;
            }
        }
    }
