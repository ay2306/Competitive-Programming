...
void solve(int test_case){
	...
	V<int> ans;
	V<int> arr;
	loop(i,1,n+1)arr.pb(i);
	for(int i = 0; i < a.size(); ++i){
		if(i+1 == a.size()){
			if(a[front] == b[b.size()-1-i] && change%2)ans.pb(1);
			if(a[front] != b[b.size()-1-i] && change%2 == 0)ans.pb(1);
			cout << ans.size() << " " <<  ans << "\n";
			return;
		}
		debug(front,a[front],i,b[b.size()-1-i],change);
		if(a[front] == b[b.size()-1-i] && change%2 == 0)ans.pb(1);
		if(a[front] != b[b.size()-1-i] && change%2 == 1)ans.pb(1);
		ans.pb(a.size()-i);
		if(change&1)front--;
		else front++;
		change++;
