...
void solve(int test_case){
	int n,m;
	cin >> n >> m;
	function<bool(int,int)> valid = [&n,&m](int i, int j)->bool{
	...
	int dx[] = {-1,1,0,0};
	int dy[] = {0,0,-1,1};
	loop(i,0,n)loop(j,0,m)if(a[i][j] == 'G')gp.pb(i,j);
	loop(i,0,n){
		loop(j,0,m){
			if(a[i][j] != 'B')continue;
			if(valid(i-1,j) && a[i-1][j] == '.')a[i-1][j] = '#';
			if(valid(i+1,j) && a[i+1][j] == '.')a[i+1][j] = '#';
			if(valid(i,j-1) && a[i][j-1] == '.')a[i][j-1] = '#';
			if(valid(i,j+1) && a[i][j+1] == '.')a[i][j+1] = '#';
		}
	}
	if(gp.size() == 0){
