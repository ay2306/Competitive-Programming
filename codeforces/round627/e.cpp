...
void solve(int test_case){
	int n,h,l,r;
	loop(i,0,2020)loop(j,0,2020)dp[i][j] = -inf;
	scanf("%d%d%d%d",&n,&h,&l,&r);
	loop(i,0,n)scanf("%lld",a+i);
	s[0] = a[0];
	loop(i,1,n)s[i]=s[i-1]+a[i];
	dp[0][0] = 0;
	loop(i,0,n){
		loop(j,0,h){
			dp[i+1][(j+a[i]-1)%h] = max(dp[i][j]+(l<=(j+a[i]-1)%h && (j+a[i]-1)%h <= r),dp[i+1][(j+a[i]-1)%h]);
			dp[i+1][(j+a[i])%h] = max(dp[i][j]+(l<=(j+a[i])%h && (j+a[i])%h <= r),dp[i+1][(j+a[i])%h]);
		}
	}
	ll ans = 0;
	loop(i,0,h+1){
