...
void dfs(int s = 1, int p = 0){
	...
	for(int &i: g[s]){
		if(i == p)continue;
		dfs(i,s);
		if(cnt[i] > 0)cnt[s]+=cnt[i];
	}
}

void setans(int s = 1, int p = 0){
	if(s != 1){
		int pa = cnt[p]+fp[p];
		if(cnt[s] > 0)pa-=cnt[s];
		if(pa > 0)fp[s]=pa;
	}
	for(auto &i: g[s])if(i != p)setans(i,s);
}
