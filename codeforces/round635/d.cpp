...
void solve(int test_case){
	...
	loop(i,0,r){
		ll x = a1[i];
		it = lower_bound(all(a2),x);
		if(it != a2.end()){
			ll mid = x+*it >> 1;
			it1 = lower_bound(all(a3),mid);
			if(it1 != a3.end()){
				ans = min(ans,sq(*it1,x)+sq(*it1,*it)+sq(*it,x));
			}
			if(it1 != a3.begin()){
				it1--;
				ans = min(ans,sq(*it1,x)+sq(*it1,*it)+sq(*it,x));
		...
		it = lower_bound(all(a3),x);
		if(it != a3.end()){
			ll mid = x+*it >> 1;
			it1 = lower_bound(all(a2),mid);
			if(it1 != a2.end()){
				ans = min(ans,sq(*it1,x)+sq(*it1,*it)+sq(*it,x));
			}
			if(it1 != a2.begin()){
