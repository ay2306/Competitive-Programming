...
void calculate(int pack, int left, int right, int lo, int hi){
	...
	if(left != right){
		calculate(pack,left,mid-1,lo,besti);
		calculate(pack,mid+1,right,besti,hi);
	}
}
...
void solve(int test_case){
	...
	loop(i,0,40000){
		loop(j,0,60)dp[j][i] = inf;
	}
	dp[0][0] = 0;
	// calculate(1,1,n,1,n);
	for(int pack = 1; pack <= k; ++pack){
		calculate(pack,1,n,1,n);
	}
	cout << -dp[k][n];
}
