...
void solve(int test_case){
	...
	cin >> b;
	V<int> bt;
	loop(i,0,m)if(!b[i])bt.pb(i);
	while(bt.size()){
		while(arr.back().second < bt.size())arr.pop_back();
		char c = arr.back().first;
		arr.pop_back();
		// cout << c << "\n";
		while(bt.size()){
			ans[bt.back()] = c;
			// cout << bt.back() << " " << 
			b[bt.back()] = -1;
			loop(i,0,m)b[i]-=abs(bt.back()-i);
			bt.pop_back();
		}
		// for(auto i: b)cout << i << " ";
		// cout << "\n";
		loop(i,0,m)if(b[i]==0)bt.pb(i);
