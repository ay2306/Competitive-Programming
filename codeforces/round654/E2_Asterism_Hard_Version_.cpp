...
void solve(int test_case){
    ...
    cin >> a;
    sort(all(a));
    ll min_x = findMnStart();
    ll mx_x = a[p-1]-1;
    loop(i,0,n){
        if(i+1 >= p)mx_x = min(mx_x,a[i]-i+p-1);
    }
	while(min_x <= mx_x){
        ll per = 1, x = mx_x;
        loop(j,0,n){
            ll choice = upper_bound(all(a),x) - a.begin();
            choice-=j;
            x++;
            if(!choice || choice >= p){
                per = 0;
    ...
    cout << max(mx_x-min_x+1,0LL) << "\n";
	for(ll i = min_x; i <= mx_x ; ++i)cout << i << " ";
}
