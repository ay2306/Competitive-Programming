...
int dp[N][N],a[N][N],maxSum[N][N][N], ans[N][N];
void solve(int test_case){
	...
	loop(i,1,n+1){
		...
		loop(j,1,m+1){
			for(int p = m/2; p; p--){
				//a[i][j] is selected at pth element making row sum%k = rem
				loop(rem,0,k){
					int prv = (rem - a[i][j]%k + k)%k;
					if(maxSum[i][p-1][prv] == -1)continue;
					maxSum[i][p][rem] = max(maxSum[i][p][rem],maxSum[i][p-1][prv]+a[i][j]);
					dp[i][rem] = max(dp[i][rem],maxSum[i][p][rem]);
				}
			}
		}
	}
	loop(i,0,n+1){
		loop(j,0,k)cout << dp[i][j] << " ";
		cout << endl;
	}
	...
	loop(i,1,n+1){
		loop(rem,0,k){
			ans[i][rem] = ans[i-1][rem];
			loop(irem,0,k){
				int prv = (rem - irem + k) % k;
				if(ans[i-1][prv] == -1 || dp[i][irem] == -1){
					continue;
				}
				ans[i][rem] = max(ans[i][rem],ans[i-1][prv] + dp[i][irem]);
			}
			// cout << ans[i][rem] << " ";
		}
		// cout << endl;
