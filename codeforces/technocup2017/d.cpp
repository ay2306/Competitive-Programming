...
void solve(int test_case){
	map<string,V<pair<int,string>>> k;
	set<string> taken;
	int n;
	cin >> n;
	vector<string> ans(n);
	...
	set<P<int,P<string,string>>> un;
	for(auto &i: k){
		if(i.second.size() == 1){
			un.insert(mp(i.second[0].first,mp(i.first,i.second[0].second)));
			continue;	
		}
		for(auto &j: i.second){
			if(taken.count(j.second)){
				cout << "NO\n";
				return;
			}
			taken.insert(j.second);
			ans[j.first] = j.second;
		}
	}
	V<set<P<int,P<string,string>>>::iterator> to_delete;
	while(un.size()){
		to_delete.clear();
		for(auto itr = un.begin(); itr != un.end(); ++itr){
			auto i = *itr;
			if(taken.count(i.S.S) && taken.count(i.S.F)){
				cout << "NO";
				return;
			}
			if(!taken.count(i.S.S) && !taken.count(i.S.F)){
			...
			if(!taken.count(i.S.F))ans[i.F] = i.S.F,taken.insert(i.S.F);
			else ans[i.F] = i.S.S,taken.insert(i.S.S);
			to_delete.pb(itr);
		}
		if(to_delete.size() == 0){
			for(auto itr = un.begin(); itr != un.end(); ++itr){
		...
		for(auto i: to_delete)un.erase(i);
