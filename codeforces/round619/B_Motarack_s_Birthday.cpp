...
void solve(int test_case){
    ...
    int mn = INT_MAX;
    loop(i,0,n){
        if(i && a[i-1] == -1 && ~a[i])mx = max(mx,a[i]);
        if(i+1 < n && a[i+1] == -1 && ~a[i])mx = max(mx,a[i]);
        if(i && a[i-1] == -1 && ~a[i])mn = min(mn,a[i]);
        if(i+1 < n && a[i+1] == -1 && ~a[i])mn = min(mn,a[i]);
    }
    if(mx == -1)return cout << "0 0\n",void();
    int k = (mx + mn) / 2;
    mx = 0;
    loop(i,0,n){
        if(a[i] == -1)a[i] = k;
