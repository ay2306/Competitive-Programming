...
struct ds{
	ll rem,q;
	ds(ll a = 0, ll b = 0):q(a),rem(b){}
	bool operator<(const ds &r)const{
		if(r.q != q)return q < r.q;
		return rem < r.rem;
	}
	bool operator==(const ds &r)const{
		return q == r.q && rem == r.rem;
	}
	bool operator>(const ds &r)const{
		if(r.q != q)return q > r.q;
		return rem > r.rem;
	}
	ds operator+(const ds &r)const{
		ll remn = rem+r.rem;
		ll qn = q+r.q + (remn>=mod);
		remn%=mod;
		return ds(qn,remn);
	}
	ds operator+(const int &x)const{
		ll remn = rem+x;
		ll qn = q + (remn>=mod);
		remn%=mod;
		return ds(qn,remn);
...
int main(){
	...
	loops(i,3,N,1){	
		dp[i][1] = dp[i-2][0] + dp[i-2][0] + dp[i-1][0] + 4;
		dp[i][0] = max(dp[i-1][0],dp[i-1][1]) + max(dp[i-2][0],dp[i-2][1]) + max(dp[i-2][0],dp[i-2][1]);
	}
