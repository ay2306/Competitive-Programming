...
struct Node{
    ...
    Node operator+(Node &r){
        Node res;
        for(int i = 0; i < 26; ++i)res.f[i] = f[i] + r.f[i];
        return res;
    }
};
...
void update(int id, char c, int node = 1, int start = 0, int end = n-1){
    if(start == end){
        tr[node].reset();
        tr[node].f[c-'a']++;
        return;
    ...
    int mid = start + end >> 1;
    if(id <= mid)update(id,c,node<<1,start,mid);
    else update(id,c,node<<1|1,mid+1,end);
    tr[node] = tr[node<<1] + tr[node<<1|1];
}

Node query(int l, int r, int node = 1, int start = 0, int end = n-1){
    if(l > end || r < start)return Node();
    if(l <= start && end <= r)return tr[node];
    int mid = start+end >> 1;
    Node left = query(l,r,node<<1,start,mid);
    Node right = query(l,r,node<<1|1,mid+1,end);
    return left + right;
}
...
int cost(int l, int r, int c){
    ...
    int len = mid - l + 1;
    // Keep left same;
    int left = len - query(l,mid).f[c-'a'] + cost(mid+1,r,char(c+1));
    int right = len - query(mid+1,r).f[c-'a'] + cost(l,mid,char(c+1));
