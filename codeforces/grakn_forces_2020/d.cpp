...
void solve(int test_case){
	...
	for(auto i: r){
		for(auto j: s){
			a.pb(max(0LL,j.F+1-i.F),max(0LL,j.S+1-i.S));
		}
	}
	sort(all(a));
	stack<int> mx;
	...
	for(int i = a.size()-1; ~i; --i){
		if(mx.top() < a[i].S)mx.push(a[i].S);
		else mx.push(mx.top());
	}
	int ans = INT_MAX, i = 0;
	for(int dx = 0; dx <= a.back().F+2; ++dx){
		while(i < a.size() && a[i].F <= dx)mx.pop(),i++;
		ans = min(ans,dx+mx.top());
	}
	cout << ans;
}
