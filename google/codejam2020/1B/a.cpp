...
void solve(int test_case){
	...
	// debug(x,y);
	bool normal = true;
	char h = 'E', hr = 'W', v = 'N', vr = 'S';
	if(x < 0)swap(h,hr);
	if(y < 0)swap(v,vr);
	x = abs(x);
	y = abs(y);
	int mx = 0;
	if(int(log2(x)) == int(log2(y)))mx = int(log2(x))+2;
	else mx=max(log2(x),log2(y))+1;
	// if(x > y){
	// 	swap(x,y);
	// 	swap(v,h);
	// 	swap(vr,hr);
	...
	loop(i,0,40){
		...
		if((((x >> (i+1))&1) != ((y >> (i+1))&1)) || ((x < (1LL << (i+1))) && (y < (1LL << (i+1))))){
			if((x>>i)&1)ans+=h;
			else ans+=v;
		}else{
			if((x>>i)&1)ans+=hr,x+=(1LL<<(i+1));
			else ans+=vr,y+=(1LL<<(i+1));
		}
		x=(x&(~(1LL<<i)));
		y=(y&(~(1LL<<i)));
		if(!x && !y)break;
