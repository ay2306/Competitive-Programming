...
void solve(int test_case){
	int r,c;
	cin >> r >> c;
	loop(i,0,50)g[i].clear(),in[i] = 0;
	V<string> a(r+1);
	loop(i,0,r)cin >> a[i];
	set<int> u;
	loop(i,0,r)loop(j,0,c)u.insert(a[i][j]-'A');
	loop(lev,1,r){
		loop(i,0,r-lev){
			loop(j,0,c){
				int ar = a[i][j]-'A';
	...
	queue<int> q;
	string ans = "";
	loop(i,0,50)if(u.count(i) && in[i] == 0)q.push(i);
	while(q.size()){
		int v = q.front();
		q.pop();
		char s = char(v+'A');
