...
void solve(int test_case){
    ...
    cin >> m >> n;
	debug(n,m);
    V<V<ld>> opt(n,V<ld>(m,0)), p(n,V<ld>(m,0));
    for(int i = 0; i < n; ++i){
        for(int j = 0; j < m; ++j){
			if(i+1 < n && j+1 < m)opt[i][j] = 0.5;
			else opt[i][j] = 1;	
		}
    }
	// cout << "\n";
	// for(auto &i: opt){for(auto &j: i)cout << j << " ";cout << endl;}
    p[0][0] = 1;
    for(int i = 1; i < n; ++i){
        for(int j = 1; j < m; ++j){
            p[i][j] += p[i-1][j]*opt[i-1][j];
            p[i][j] += p[i][j-1]*opt[i][j-1];
        }
    }
    int r1,r2,c1,c2;
    // cin >> c1 >> r1 >> c2 >> r2;
	...
	for(int i = 0; i < n; ++i){
        for(int j = 0; j < m; ++j){
            if(i-1 >= 0)p[i][j] += p[i-1][j]*opt[i-1][j];
            if(j-1 >= 0)p[i][j] += p[i][j-1]*opt[i][j-1];
	...
	cout << ans << "\n";
	// for(auto &i: p){for(auto &j: i)printf("%3.3Lf ");cout << endl;}
    printf("%.10Lf\n",1-ans);
}
