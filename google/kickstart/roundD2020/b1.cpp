...
void solve(int test_case){
	...
	V<int> a(n);
	cin >> a;
	V<V<ll>> dp(n,V<ll>(5,INT_MAX));
	fill(all(dp[0]),0);
	loop(i,1,n){
		if(a[i] > a[i-1]){
			loop(cur,1,5){
				loop(pr,1,cur)dp[i][cur] = min(dp[i][cur],dp[i-1][pr]);
				loop(pr,cur,5)dp[i][cur] = min(dp[i][cur],dp[i-1][pr]+1);
			}
		}
		if(a[i] == a[i-1]){
			loop(cur,1,5){
				loop(pr,1,cur)dp[i][cur] = min(dp[i][cur],dp[i-1][pr]+1);
				loop(pr,cur,cur+1)dp[i][cur] = min(dp[i][cur],dp[i-1][pr]);
				loop(pr,cur+1,5)dp[i][cur] = min(dp[i][cur],dp[i-1][pr]+1);
			}
		}
		if(a[i] < a[i-1]){
			loop(cur,1,5){
				loop(pr,1,cur+1)dp[i][cur] = min(dp[i][cur],dp[i-1][pr]+1);
				loop(pr,cur+1,5)dp[i][cur] = min(dp[i][cur],dp[i-1][pr]);
			}
		}
	}
	cout << *min_element(1+all(dp[n-1])) << "\n";
}
