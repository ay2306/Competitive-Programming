...
void solve(int test_case){
	...
	// dp[i][j] = max money we can make if we choose j items till i
	// V<V<ll>> a(n,V<ll>(k));
	loop(i,1,n+1)loop(j,1,k+1)cin >> a[i][j];
	loop(i,1,k+1)dp[1][i] = dp[1][i-1]+a[1][i];
	loop(i,2,n+1){
		loop(j,1,p+1){
			ll s = 0;
			loop(o,0,k+1){
				if(o > j)break;
				s+=a[i][o];
				dp[i][j] = max(dp[i-1][j-o]+s,dp[i][j]);
			}
		}
	}
