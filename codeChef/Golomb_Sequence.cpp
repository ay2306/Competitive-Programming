...
void solve(int test_case){
    ...
    ll occFreqFreq = 1;
    ll savedOccFreq = 1;
    ll savedOccFreqFreq = 1;
    map<ll,ll> s;
    s[0] = 0;
    int bunty = 1;
    while(arr.size()){
        // debug(cur,occ,total,sum,arr.size());
        while(arr.size() && total + occ >= *arr.begin()){
            ll req = *arr.begin() - total;
            s[*arr.begin()] = (sum + cur*cur%mod*req%mod)%mod;
            arr.erase(arr.begin());
        }
        sum = (sum + cur * cur % mod * occ % mod)%mod;
        total+=occ;
        cur++;
        occFreq--;
        if(occFreq){
