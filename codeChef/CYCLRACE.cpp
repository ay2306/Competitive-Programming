...
struct chtDynamics{
    ...
    struct line{
        ...
        //m = speed;
        //b = distance_covered_atStart - time_started*speed;
...
struct cyclist{
    ...
    ll distance;
    ll time;
    cyclist():speed(0),distance(0),time(0){}
};
...
int main(){
    ...
    while(q--){
        ...
        if(t == 1){
            ...
            ll tm,cyc,sp;
            cin >> tm >> cyc >> sp;
            arr[cyc].distance += (tm-arr[cyc].time)*arr[cyc].speed;
            arr[cyc].speed = sp;
            arr[cyc].time = tm;
            o.addLine(arr[cyc].speed,arr[cyc].distance - arr[cyc].time*arr[cyc].speed);
        }else{
            ll tm;
            cin >> tm;
            cout << o.getBest(tm) << "\n";
