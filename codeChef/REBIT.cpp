...
vector<ll> newVector(){
	return vector<ll> (4,MODINV_4);
}

string generatePostfix(string &a){
	string ans = "";
	stack<char> s;
	for(auto &i: a){
		if(i == '&' || i == '|' || i == '^'){
...
vector<ll> EXOR(vector<ll> &a, vector<ll> &b){
	vector<ll> c(4,0);
	c[0] += a[0]*b[3]%MOD;
	c[0]%=MOD;
	c[0] += a[3]*b[0]%MOD;
...
void solve(int test_case){
	...
	auto j = s.top();
	s.pop();
	cout << j[3] << " " << j[0] << " " << j[1] << " " << j[2] << "\n";
}

int main(){
	FAST
	MODINV_4 = power(4,MOD-2);
	int t = 1;
	cin >> t;
