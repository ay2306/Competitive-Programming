...
int solve(int test_case){
	...
	if(s.size() == 0)return cout << 0 << "\n",0;
	for(int i = s[0].second+1; i < a.size(); ++i){
		if(a[i] != b[i]){
			if(s.back().second + 1 == i)s.rbegin()->second++;
			else s.emplace_back(i,i);
		}
	}
	ll ch = 0, k = s.size(),ans;
	multiset<ll> m;
	for(int i = 0; i < s.size(); ++i){
		ch+=(s[i].second - s[i].first + 1);
		if(i)m.insert(s[i].first-s[i-1].second-1);
	}
	ans = k*ch;
	while(m.size()){
		k--;
