...
ll tree[N<<2];
int n;
void update(int id, ll val, int node = 1, int start = 1, int end = n){
	if(start == end){
		tree[node] = val;
		return;
	}
	int mid = start+end >> 1;
	if(mid >= id)update(id,val,node<<1,start,mid);
...
ll query(int l, int r, int node = 1, int start = 1, int end = n){
	...
	int mid = start+end >> 1;
	ll p1= query(l,r,node<<1,start,mid);
	ll p2= query(l,r,node<<1|1,mid+1,end);
	return (p1+p2)%mod;
}
