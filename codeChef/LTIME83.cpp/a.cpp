...
void solve(int test_case){
	...
	arr.pb(1,1);
	loop(i,1,n){
		if(arr.back().F == a[i])arr.rbegin()->second++;
		else arr.pb(a[i],1);
	}
	arr.pb(inf,1);
	ll ans = 0;
	...
	ll ways = 1;
	for(auto &k: arr){
		if(cnt == k.first)n -= k.second;
		ans = (ans + cnt*(ways*power(2,n)%mod)%mod)%mod;
		ways = ways*((power(2,k.second)-1+mod)%mod)%mod;
		if(cnt != k.first)break;
		else cnt++;
	}
	cout << ans << "\n";
}
