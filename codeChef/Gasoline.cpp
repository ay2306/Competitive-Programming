...
void solve(int test_case){
    int n; cin >> n;
    V<ll> f(n),c(n),dp(n+1); cin >> f >> c;
    multiset<PLL> m,cst;
    cst.emplace(c[0],0);
    m.emplace(f[0],0);
    loop(i,1,n+1){
        while(m.size() && m.begin()->F < i){
            auto it = cst.find(make_pair(c[m.begin()->second],m.begin()->second));
            cst.erase(it);
            m.erase(m.begin());
        }
        dp[i] = dp[cst.begin()->second] + (i - cst.begin()->second)*cst.begin()->first;
        cst.emplace(c[i],i);
        m.emplace(f[i]+i,i);
    }
    cout << dp << "\n";
