...
void solve(int test_case){
    ...
    V<int> pref(n),suff(n);
    V<int> pCost(n),sCost(n);
    V<PII> mx(n,PII(0,0));
    for(int i = 0; i < n; ++i){
        pref[i] = a[i].S;
        if(i)pref[i] += pref[i-1];
        pCost[i] = pref[i] - a[i].F;
        ans = max(ans,pCost[i]);
    }
    for(int i = n-1; ~i; --i){
        suff[i] = a[i].S;
        if(i+1 < n)suff[i] += suff[i+1];
        sCost[i] = suff[i] - (c - a[i].F);
        ans = max(ans,sCost[i]); 
    }
    for(int i = n-2; ~i; --i){
        mx[i] = mx[i+1];
        mx[i].F = max(mx[i].F,sCost[i+1]);
        mx[i].S = max(mx[i].S,sCost[i+1] - (c - a[i+1].F));
    }
    for(int i = 0; i < n; ++i){
        ans = max({ans,pCost[i] + mx[i].F - a[i].F, pCost[i] + mx[i].S});
    }
    cout << ans ;
}
