...
vector<ll> p1,p2,p,arr;
ll n,t;

void allSubset(int index, ll sum){
    if(index == arr.size()){
        p.emplace_back(sum);
        return;
    }
    allSubset(index+1,sum+arr[index]);
    allSubset(index+1,sum);
}
...
void solve(int test_case){
    ...
    for(auto i: p1){
        if(i > t)break;
        ll rem = t - i;
        auto j = upper_bound(all(p2),rem);
        if(*prev(j)+i <= t)
        ans = max(ans,*prev(j)+i);
    }
    for(auto i: p2){
        if(i > t)break;
        ll rem = t - i;
        auto j = upper_bound(all(p1),rem);
        if(*prev(j)+i <= t)
        ans = max(ans,*prev(j)+i);
    }
    cout << ans;
