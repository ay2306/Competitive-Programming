...
void solve(int test_case){
    ll x1,x2,y1,y2;
    cin >> x1 >> y1 >> x2 >> y2;
    if(x1 == x2 && y1 == y2)return void(cout << 0);
    if(x1+y1 == x2+y2)return void(cout << 1);
    if(x1-y1 == x2-y2)return void(cout << 1);
    if(abs(x2-x1) + abs(y2-y1) <= 3)return void(cout << 1);
    loop(dx,-3,4){
        loop(dy,-3,4){
            if(abs(dx)+abs(dy) > 3)continue;
            int x = x1+dx,y=y+dy;
            if(x+y1 == x+y2)return void(cout << 2);
            if(x-y1 == x-y2)return void(cout << 2);
            if(abs(x2-x) + abs(y2-y) <= 3)return void(cout << 2);
        }
    }
}
