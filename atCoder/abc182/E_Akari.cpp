...
void solve(int test_case){
    int a,b,h,w,n,m;
    cin >> h >> w >> n >> m;
    V<V<int>> arr(h+5,V<int>(w+5));
    V<V<int>> light(h+5,V<int>(w+5));
    V<PII> q;
    loop(i,0,n){
        cin >> a >> b;
        arr[a][b] = 1;
    ...
    loop(i,0,n){
        tie(a,b) = q[i];
        arr[a][b] = 1;
        for(int i = a-1; i && arr[i][b] == 0 ; --i)arr[i][b] = 1;
    }
    loop(i,1,h+1)
        loop(j,1,w+1)
            if(arr[i][j] == 1)
    ...
    loop(i,0,n){
        tie(a,b) = q[i];
        arr[a][b] = 1;
        for(int i = a+1; i <= h && arr[i][b] == 0 ; ++i)arr[i][b] = 1;
    }
    loop(i,1,h+1)
        loop(j,1,w+1)
            if(arr[i][j] == 1)
    ...
    loop(i,0,n){
        tie(a,b) = q[i];
        arr[a][b] = 1;
        for(int i = b-1; i && arr[a][i] == 0 ; --i)arr[a][i] = 1;
    }
    loop(i,1,h+1)
        loop(j,1,w+1)
            if(arr[i][j] == 1)
    ...
    loop(i,0,n){
        tie(a,b) = q[i];
        arr[a][b] = 1;
        for(int i = b+1; i <= w && arr[a][i] == 0 ; ++i)arr[a][i] = 1;
    }
    loop(i,1,h+1)
        loop(j,1,w+1)
            if(arr[i][j] == 1)
